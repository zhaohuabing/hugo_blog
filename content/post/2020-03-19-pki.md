...
* 消息接受者验证发送者身份： isValid := isEqual(decrypt(pk, message),hash(message))
...
在前面我们讲到可以通过数字签名来证明一个人或者组织拥有一个公钥对应的私钥，因此我们可以把一个公钥看作一个数字身份标识，如果一个人可以发出使用该标识(公钥)签名的数据,则说明该用户是该公钥及对应的私钥的拥有者。
